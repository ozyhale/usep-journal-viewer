    ...
    <tbody>
        {foreach from=$name key=k item=i}
           <tr> 
                ...
                <td><center><button class="btn btn-danger" onclick="onclick_delete_users('{$name[$k]['id']}','{$name[$k]['lastname']}','{$name[$k]['firstname']}','{$name[$k]['middle_initial']}')"><i class="icon-remove icon-white"></button></center></td>
            </tr>
        {/foreach}    
